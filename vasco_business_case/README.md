...
A daily survey dataset is received as a CSV file. This pipeline ingests it, applies business rules, and produces clean, analysis-ready tables. The two main questions the business wants to answer are:
...
vasco_business_case/
  ...
  docs/
    architecture.pdf      [diagram to be added]
```
...
Variance is calculated across all numeric survey columns. Three columns are excluded from the ranking: `Respondent` (a row ID), `ConvertedSalary`, and `salary_yearly`. Their variance is driven by monetary scale and currency differences, not by how respondents answered a question, so including them would make the ranking meaningless.

`fact_variance` is a pre-calculated snapshot of the full dataset so the business has an immediate answer. For any filtered analysis (e.g. variance only for full-time employees), `fact_survey` should be used directly.

### Salary vs. Years Coding (correlation)

`YearsCoding` is stored as a text range (e.g. `"6-8 years"`). Each range is mapped to its midpoint (e.g. `7`) to make the correlation calculation possible. The Pearson correlation is then calculated between `years_coding_numeric` and `ConvertedSalary`. USD-converted salary is used rather than `salary_yearly` to ensure fair comparison across countries.
...
This pipeline runs locally using Python and pandas. The same logic would translate to Azure without structural changes; only the paths and execution environment change.

### Storage

Each layer would be stored in **Azure Data Lake Storage Gen2 (ADLS Gen2)** as Parquet files, partitioned by ingestion date. The three-layer structure (Raw, Prepared, Staging) maps directly to separate containers or folders in the same storage account.
...
A **Fabric Data Factory pipeline** (or Azure Data Factory) would trigger the notebook daily on a schedule. The input file paths would be passed as parameters to the notebook, which is why they are defined in a separate parameters cell at the top.
...
- **Fabric Lakehouse with SQL Analytics Endpoint** - if the pipeline writes Delta format instead of Parquet, Fabric automatically exposes the files as queryable tables via a SQL endpoint with no DDL required. The `create_tables.sql` file would serve as schema documentation only in this case.
